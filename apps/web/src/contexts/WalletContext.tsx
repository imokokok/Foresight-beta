...
export function WalletProvider({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (lastWalletType) {
          ...
          const discovered = discoveredProviders.find(
            (d) => providerTypeMap.get(d.provider) === lastWalletType
          );
          if (discovered) {
  ...
  const handleChainChanged = (chainId: string | number) => {
    ...
    const provider =
      typeof window !== "undefined"
        ? currentProviderRef.current || (window as any).ethereum || (window as any).BinanceChain
        : null;
        ...
        try {
          ...
          try {
            ...
            if (walletState.chainId) {
              try {
                chainIdNum = parseInt(walletState.chainId, 16);
              } catch {}
            }
          }
        }
  ...
  const connectWallet = async (walletType?: WalletType) => {
    ...
    try {
      ...
      if (walletType) {
        ...
        const discovered = discoveredProviders.find(
          (d) => providerTypeMap.get(d.provider) === walletType
        );
        if (discovered) {
          targetProvider = discovered.provider;
        }
