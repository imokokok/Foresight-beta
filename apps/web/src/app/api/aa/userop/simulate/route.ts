...
export async function POST(req: NextRequest) {
  try {
    const ip = getIP(req);
    const limitResult = await checkRateLimit(ip, RateLimits.moderate, "aa_userop_simulate_ip");
    ...
    const relayerUrl = new URL("/aa/userop/simulate", relayerBase);
    ...
    return proxyJsonResponse(relayerRes, {
      ...
      onResult: async ({ ok, status, json }) => {
        ...
        await logApiEvent("aa_userop_simulate_failed", {
    ...
    try {
      ...
      await logApiEvent("aa_userop_simulate_exception", {
        owner: owner ? owner.slice(0, 10) : "",
        message: String(error?.message || error).slice(0, 300),
      });
    } catch {}
    logApiError("POST /api/aa/userop/simulate", error);
