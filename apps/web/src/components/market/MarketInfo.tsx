  ...
  return (
    <div className="bg-white border border-purple-100 rounded-3xl overflow-hidden shadow-sm relative">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wider">
                    {tMarket("discipline.kicker")}
                  </div>
                  <div className="text-base font-bold text-gray-900">
                    {tMarket("discipline.title")}
                  </div>
                  <div className="mt-2 text-sm text-gray-600 leading-relaxed">
                    {tMarket("discipline.body")}
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setDisciplineOpen(true)}
                  className="shrink-0 inline-flex items-center justify-center px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-200 to-pink-300 text-purple-800 text-xs font-bold border border-purple-200 shadow-md shadow-purple-200/50 hover:from-purple-400 hover:to-pink-400 hover:text-white transition-all"
                >
                  {tMarket("discipline.cta")}
                </button>
              </div>
              <div className="mt-3 text-xs text-gray-500">{tMarket("discipline.hint")}</div>
            </div>
          </div>
        )}
      ...
      <CreateFlagModal
        ...
        onSuccess={() => {}}
        defaultTitle={formatTranslation(tMarket("discipline.defaultTitle"), {
          title: prediction.title || "",
        })}
        defaultDesc={formatTranslation(tMarket("discipline.defaultDesc"), {
          title: prediction.title || "",
        })}
